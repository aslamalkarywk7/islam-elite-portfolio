    ...
    <div className="h-screen flex flex-col bg-background">
      <header className="h-16 border-b border-border flex items-center justify-between px-4 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{onlineUsers.length} متصل الآن</p>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="icon" className="hidden lg:flex" onClick={() => setShowSidebar(!showSidebar)}>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem className="gap-2"><Settings className="w-4 h-4" /><span>إعدادات الغرفة</span></DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem className="gap-2 text-destructive"><LogOut className="w-4 h-4" /><span>مغادرة الغرفة</span></DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </header>
      <div className="flex-1 flex overflow-hidden">
        {showMobileSidebar && <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-40 lg:hidden" onClick={() => setShowMobileSidebar(false)} />}
        <div className={`fixed inset-y-0 right-0 w-72 bg-card border-l border-border z-50 transform transition-transform lg:hidden ${showMobileSidebar ? "translate-x-0" : "translate-x-full"}`}>
          <div className="h-16 flex items-center justify-between px-4 border-b border-border">
            <h2 className="font-semibold">المتصلون</h2>
            <Button variant="ghost" size="icon" onClick={() => setShowMobileSidebar(false)}><X className="w-5 h-5" /></Button>
          </div>
          <UserListSidebar onlineUsers={onlineUsers} offlineUsers={offlineUsers} onUserClick={handleUserClick} />
        </div>
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="border-t border-border p-4 flex-shrink-0">
            <div className="max-w-4xl mx-auto relative">
              {showEmojiPicker && <div className="absolute bottom-full mb-2 right-0"><EmojiPicker onSelect={handleEmojiSelect} onClose={() => setShowEmojiPicker(false)} /></div>}
              <div className="flex items-center gap-2 bg-card rounded-2xl border border-border p-2">
                <Button variant="ghost" size="icon" className="flex-shrink-0" onClick={() => setShowEmojiPicker(!showEmojiPicker)}><Smile className="w-5 h-5 text-muted-foreground" /></Button>
                <Input ref={inputRef} value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown} placeholder="اكتب رسالتك هنا..." className="flex-1 border-0 bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0" />
                <Button onClick={handleSendMessage} disabled={!inputValue.trim()} className="flex-shrink-0 gap-2"><span className="hidden sm:inline">إرسال</span><Send className="w-4 h-4" /></Button>
              </div>
            </div>
          </div>
        </div>
        {showSidebar && <div className="hidden lg:block w-72 border-r border-border bg-card flex-shrink-0"><div className="h-16 flex items-center px-4 border-b border-border"><h2 className="font-semibold">المتصلون ({onlineUsers.length})</h2></div><UserListSidebar onlineUsers={onlineUsers} offlineUsers={offlineUsers} onUserClick={handleUserClick} /></div>}
      </div>
      {privateChat && <PrivateMessagePopup user={privateChat} currentUser={currentUser} onClose={() => setPrivateChat(null)} />}
    </div>
  );
}
